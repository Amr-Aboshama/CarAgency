...
//=======
        ...
        {
            string query = "update Car set Status =" + status + " where ChassisID ='" + id + "';";
            return dbMan.ExecuteNonQuery(query);
        }

        public DataTable SelectAvailabeCars(string catname)
        {
            string query = "select c.ChassisID,C.Color,t.Name from Car c,Store t where c.StoreID = t.StoreID and c.CatName ='" + catname + "' and Status = 1;";
            return dbMan.ExecuteReader(query);
        }
        ...
        {
            string query = "SELECT ChassisID,Color,Name from Car c,Store s,Category t "
        ...
        {
            string query = "select Brand,Type,r.CatName as Category,Model,Color,Specs,ChassisID "
        ...
        public int InsertCarIntoStore(string id,string storeid)
        {
            string query = "update Car set "
                         + "StoreID ='" + storeid + "' where ChassisID ='" + id + "';";
            return dbMan.ExecuteNonQuery(query);
        }
...
//>>>>>>> Nehal
        ...
        public DataTable[] selectSaleInfo(int id)
        {
            ...
            query =
                "select c.catName as 'Category', c.Brand, c.Type, c.Model, c.primaryCashPrice as 'Primary Cash Price', c.Currency, " +
                    "r.ChassisID as 'Chasis', r.motorNum as 'Motor', r.Color, " +
                    "t.Name as 'Store Name', t.Address as 'Store Address' " +
                "from sales s left outer join car r on s.carID = r.ChassisID " +
        ...
        public DataTable[] selectPurchaseInfo(int id)
        {
            ...
            query =
                "select c.catName as 'Category', c.Brand, c.Type, c.Model, c.primaryCashPrice as 'Primary Cash Price', c.Currency, " +
                        "r.ChassisID as 'Chasis', r.motorNum as 'Motor', r.Color, " +
                        "t.Name as 'Store Name', t.Address as 'Store Address' " +
                "from purchases p left outer join car r on p.carID = r.ChassisID " +
                             "left outer join category c on r.catName = c.catName " +
                             "left outer join store t on r.storeID = t.storeID " +
                "where purchaseID = " + Convert.ToString(id)
                ;
        ...
        public int addCallRecord(int custID, decimal empID, string text)
        {
            string query =
        ...
        public string getEmpJob(decimal empID)
        {
