...
namespace Car_Agency
{
	public partial class SPDetails : MetroForm
	{
		...
		public SPDetails(bool isSale, int id)
		{
			...
			int eID = controllerObj.getEmpIDByUserName(Login.userName);

			job = controllerObj.getJob(eID);

			
			 if (String.Equals(job, "Treasury Manager", StringComparison.OrdinalIgnoreCase))
			{
				DataTable dt = controllerObj.getSafes();
				metroComboBox1.DataSource = dt;
				metroComboBox1.ValueMember = "BankID";
				metroComboBox1.DisplayMember = "BankName";
			}
			else if (String.Equals(job, "Bank Manager", StringComparison.OrdinalIgnoreCase))
			{
				DataTable dt = controllerObj.getBanks();
				metroComboBox1.DataSource = dt;
				metroComboBox1.ValueMember = "BankID";
				metroComboBox1.DisplayMember = "BankName";
			}
		...
		{
			if (controllerObj.addTransaction() != 0)
			{
